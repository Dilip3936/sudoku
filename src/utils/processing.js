...
function cleanBorders(cellBin, cv) {
  ...
  // Pad by 1px on each side so border-touching contours are detected by RETR_EXTERNAL
  let mask = new cv.Mat();
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    for (let j = 0; j < cnt.rows; j++) {
      // Each contour point is stored as (x, y) accessed via intAt(row, col)
      const x = cnt.intAt(j * 2);     // x-coordinate
      const y = cnt.intAt(j * 2 + 1); // y-coordinate
    ...
    if (touchesBorder) {
      // Fill this contour with black (erase it from the cell)
      cv.drawContours(cellClean, contours, i, new cv.Scalar(0), -1);
    }
    cnt.delete();
  }

  // Light erosion to remove remaining 1px border noise
...
export function processCells(cells, cv) {
  return cells.map(cell => {
    ...
    cellBin.delete(); // Free intermediate mat
    let cellResized = resizeCell(cellClean, 32, cv);
    cellClean.delete(); // Free intermediate mat
    return cellResized;
  });
}
